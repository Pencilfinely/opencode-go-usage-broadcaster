...
明细成功时，模型排行显示前五个模型，其余模型合并为“其他”。正文安全预算为 18,000 字符；超过预算时会使用兼容版，它只移除顶部的大型 24 小时趋势图，仍保留完整的 24 小时精确数据和每小时迷你条。如果采集在限制内没有完成，数值会以“至少”标识，仪表盘也只代表已经采集到的最新记录。明细不可用时，三个额度仍会显示，仪表盘会说明原因和下一步。
